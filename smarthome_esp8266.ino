...
void loadMQTTConfig() {
  ...
  if (calc != mqttConfig.checksum) {
    // Default values (match the dashboard's example)
...
String webpage() {
  ...
  p += ".fan-card{flex:1 1 auto;}.lights-card{flex:0 0 auto;}.wifi-card{flex:0 0 auto;background:transparent;box-shadow:none;padding:0;margin:5px 0 0 0;}";
  ...
  p += ".btn-remote:hover{background:#7c3aed;}";
  ...
  p += "document.getElementById('fanSlider').disabled=!data.fanEnabled;const fanIcon=document.getElementById('fanIcon');";
...
String remotePage() {
  ...
  p += ".back-btn{background:#475569;margin-top:8px;}</style>";
...
void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
  ...
  if (cmd.startsWith("SPEED_")) {
    ...
    publishMqttStatus(); // publish current state
    changed = false;
  }
